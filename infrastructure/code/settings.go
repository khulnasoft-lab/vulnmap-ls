...
func getCodeEnablementUrl() string {
	api := config.CurrentConfig().VulnmapApi()
	...
	return apiUrl.String() + "manage/vulnmap-code?from=" + integrationName
}

func isSingleTenant(url *url.URL) bool {
	return strings.HasPrefix(url.Host, "app") && strings.HasSuffix(url.Host, "vulnmap.khulnasoft.com")
}
