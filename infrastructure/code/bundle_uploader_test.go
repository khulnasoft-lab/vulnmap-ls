...
func Test_Bundler_Upload(t *testing.T) {
	...
	t.Run("adds files to bundle", func(t *testing.T) {
		vulnmapCodeService := &FakeVulnmapCodeClient{}
		var bundleUploader = BundleUploader{VulnmapCode: vulnmapCodeService, instrumentor: performance.NewInstrumentor()}
		...
		_, err := bundleUploader.Upload(context.Background(),
			Bundle{VulnmapCode: vulnmapCodeService, missingFiles: []string{documentURI}},
			bundleFileMap)

		assert.Equal(t, 1, vulnmapCodeService.TotalBundleCount)
		assert.NoError(t, err)
	})

	t.Run("when loads of files breaks down in 4MB bundles", func(t *testing.T) {
		vulnmapCodeService := &FakeVulnmapCodeClient{}
		var bundler = BundleUploader{VulnmapCode: vulnmapCodeService, instrumentor: performance.NewInstrumentor()}
		...
		_, err := bundler.Upload(context.Background(),
			Bundle{VulnmapCode: vulnmapCodeService, missingFiles: missingFiles},
			bundleFileMap)

		assert.True(t, vulnmapCodeService.HasExtendedBundle)
		assert.Equal(t, 2, vulnmapCodeService.TotalBundleCount)
		assert.Equal(t, 2, vulnmapCodeService.ExtendedBundleCount)
		assert.Nil(t, err)
	})
}
...
func Test_IsSupportedLanguage(t *testing.T) {
	...
	bundler := NewBundler(vulnmapCodeMock, performance.NewInstrumentor())
	...
	t.Run("should cache supported extensions", func(t *testing.T) {
		...
		_, _ = bundler.isSupported(context.Background(), path)
		assert.Len(t, vulnmapCodeMock.Calls, 1)
	})
}
