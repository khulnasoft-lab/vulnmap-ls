...
func TestCreateBundle(t *testing.T) {
	t.Run(
		"when < maxFileSize creates bundle", func(t *testing.T) {
			vulnmapCodeMock, dir, c, file := setupCreateBundleTest(t, "java")
			...
			assert.Len(t, vulnmapCodeMock.GetAllCalls(CreateBundleOperation), 1, "bundle should called createBundle once")
		},
	)
	...
	t.Run(
		"when too big ignores file", func(t *testing.T) {
			...
			assert.Len(t, vulnmapCodeMock.GetAllCalls(CreateBundleOperation), 0, "bundle shouldn't have called createBundle")
		},
	)
	...
	t.Run(
		"when unsupported ignores file", func(t *testing.T) {
			vulnmapCodeMock, dir, c, file := setupCreateBundleTest(t, "unsupported")
			fd, err := os.Create(file)
			t.Cleanup(
				func() {
					_ = fd.Close()
	...
	t.Run("includes config files", func(t *testing.T) {
		configFile := ".test"
		vulnmapCodeMock := &FakeVulnmapCodeClient{
			ConfigFiles: []string{configFile},
		}
		scanner := New(
			NewBundler(vulnmapCodeMock, performance.NewInstrumentor()),
			&vulnmap_api.FakeApiClient{CodeEnabled: true},
...
func setupCreateBundleTest(t *testing.T, extension string) (*FakeVulnmapCodeClient, string, *Scanner, string) {
	testutil.UnitTest(t)
	dir := t.TempDir()
	vulnmapCodeMock, c := setupTestScanner(t)
	file := filepath.Join(dir, "file."+extension)
	return vulnmapCodeMock, dir, c, file
}

func setupTestScanner(t *testing.T) (*FakeVulnmapCodeClient, *Scanner) {
	vulnmapCodeMock := &FakeVulnmapCodeClient{}
	...
	return vulnmapCodeMock, scanner
}
...
func TestUploadAndAnalyze(t *testing.T) {
	...
	t.Run(
		"should create bundle when hash empty", func(t *testing.T) {
			...
			params := vulnmapCodeMock.GetCallParams(0, CreateBundleOperation)
	...
	t.Run(
		"should retrieve from backend", func(t *testing.T) {
			...
			assert.GreaterOrEqual(t, len(issues[0].CodeActions), len(FakeIssue.CodeActions)) // Some codeactions are added by the scanner (e.g. Autofix, Vulnmap Learn)

			// verify that extend bundle has been called on backend service with additional file
			params := vulnmapCodeMock.GetCallParams(0, RunAnalysisOperation)
			assert.NotNil(t, params)
...
func Test_Scan(t *testing.T) {
	t.Run("Should update changed files", func(t *testing.T) {
		testutil.UnitTest(t)
		// Arrange
		vulnmapCodeMock, scanner := setupTestScanner(t)
		wg := sync.WaitGroup{}
		...
		// Assert
		allCalls := vulnmapCodeMock.GetAllCalls(RunAnalysisOperation)
	...
	t.Run("Shouldn't run if Sast is disabled", func(t *testing.T) {
		...
		c := New(
			NewBundler(vulnmapCodeMock, performance.NewInstrumentor()),
			&vulnmap_api.FakeApiClient{CodeEnabled: false},
			error_reporting.NewTestErrorReporter(),
			ux2.NewTestAnalytics(),
			nil,
			notification.NewNotifier(),
...
func Test_IsEnabled(t *testing.T) {
	...
	t.Run(
		"should return true if Vulnmap Code is generally enabled", func(t *testing.T) {
	...
	t.Run(
		"should return true if Vulnmap Code Quality is enabled", func(t *testing.T) {
			config.CurrentConfig().SetVulnmapCodeEnabled(false)
			config.CurrentConfig().EnableVulnmapCodeQuality(true)
			config.CurrentConfig().EnableVulnmapCodeSecurity(false)
			enabled := scanner.IsEnabled()
			assert.True(t, enabled)
		},
	)
	t.Run(
		"should return true if Vulnmap Code Security is enabled", func(t *testing.T) {
			config.CurrentConfig().SetVulnmapCodeEnabled(false)
			config.CurrentConfig().EnableVulnmapCodeQuality(false)
			config.CurrentConfig().EnableVulnmapCodeSecurity(true)
			enabled := scanner.IsEnabled()
			assert.True(t, enabled)
		},
	)
	t.Run(
		"should return false if Vulnmap Code is disabled and Vulnmap Code Quality and Security are not enabled",
		func(t *testing.T) {
			config.CurrentConfig().SetVulnmapCodeEnabled(false)
			config.CurrentConfig().EnableVulnmapCodeQuality(false)
			config.CurrentConfig().EnableVulnmapCodeSecurity(false)
...
func TestUploadAnalyzeWithAutofix(t *testing.T) {
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			vulnmapCodeMock := &FakeVulnmapCodeClient{}
			vulnmapCodeMock.NoFixSuggestions = true
...
func Test_SastApiCall(t *testing.T) {
	...
	t.Run("should call the API to check enablement if Vulnmap Code is enabled", func(t *testing.T) {
	...
	t.Run("should return an error if Vulnmap Code is enabled and the API returns an error", func(t *testing.T) {
		config.CurrentConfig().SetVulnmapCodeEnabled(true)
		apiClient.ApiError = &vulnmap_api.VulnmapApiError{}
	...
	t.Run("should return an error if Vulnmap Code is enabled and API SAST is disabled", func(t *testing.T) {
		config.CurrentConfig().SetVulnmapCodeEnabled(true)
		apiClient.ApiError = nil
