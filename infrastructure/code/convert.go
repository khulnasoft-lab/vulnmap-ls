...
func (c *exampleCommit) toReference() (reference vulnmap.Reference) {
	...
	return vulnmap.Reference{Title: c.description, Url: commitURL}
}
...
func issueSeverityToMarkdown(severity vulnmap.Severity) string {
	switch severity {
...
func (s *SarifResponse) toIssues(baseDir string) (issues []vulnmap.Issue, err error) {
	runs := s.Sarif.Runs
	if len(runs) == 0 {
		return issues, nil
	}
	...
	for _, result := range r.Results {
		for _, loc := range result.Locations {
			...
			exampleFixes := make([]vulnmap.ExampleCommitFix, 0, len(exampleCommits))
			for _, commit := range exampleCommits {
				commitURL := commit.fix.CommitURL
				commitFixLines := make([]vulnmap.CommitChangeLine, 0, len(commit.fix.Lines))
				for _, line := range commit.fix.Lines {
					commitFixLines = append(commitFixLines, vulnmap.CommitChangeLine{
						Line:       line.Line,
						LineNumber: line.LineNumber,
						LineChange: line.LineChange})
				}

				exampleFixes = append(exampleFixes, vulnmap.ExampleCommitFix{
					CommitURL: commitURL,
			...
			isSecurityType := true
			if issueType == vulnmap.CodeQualityIssue {
...
func (r *result) getMarkers(baseDir string) ([]vulnmap.Marker, error) {
	markers := make([]vulnmap.Marker, 0)
	...
	for i, arg := range r.Message.Arguments {
		...
		positions := make([]vulnmap.MarkerPosition, 0)
		for _, match := range indices {
			...
			if err != nil {
				...
				return []vulnmap.Marker{}, err
			}
			positions = append(positions, vulnmap.MarkerPosition{
				Rows: [2]int{startLine, endLine},
				Cols: [2]int{startCol, endCol},
				File: filePath,
			})
		...
		if strFindResult := strRegex.FindStringSubmatch(arg); len(strFindResult) > 1 {
			...
			// write the marker
			markers = append(markers, vulnmap.Marker{
...
func createAutofixWorkspaceEdit(absoluteFilePath string, fixedSourceCode string) (edit vulnmap.WorkspaceEdit) {
	singleTextEdit := vulnmap.TextEdit{
		Range: vulnmap.Range{
	...
	edit.Changes = make(map[string][]vulnmap.TextEdit)
	edit.Changes[absoluteFilePath] = []vulnmap.TextEdit{singleTextEdit}
	return edit
}
