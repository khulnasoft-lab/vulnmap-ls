...
import (
	...
	performance2 "github.com/khulnasoft-lab/vulnmap-ls/domain/observability/performance"
	"github.com/khulnasoft-lab/vulnmap-ls/domain/vulnmap"
	"github.com/khulnasoft-lab/vulnmap-ls/infrastructure/code/encoding"
)

const (
	completeStatus     = "COMPLETE"
	codeDescriptionURL = "https://docs.vulnmap.khulnasoft.com/scan-applications/vulnmap-code/security-rules-used-by-vulnmap-code"
)

var (
	issueSeverities = map[string]vulnmap.Severity{
		"3":       vulnmap.High,
		"2":       vulnmap.Medium,
		"warning": vulnmap.Medium, // Sarif Level
		"error":   vulnmap.High,   // Sarif Level
	}
)

var codeApiRegex = regexp.MustCompile(`^(deeproxy\.)?`)

func issueSeverity(vulnmapSeverity string) vulnmap.Severity {
	sev, ok := issueSeverities[vulnmapSeverity]
	if !ok {
		return vulnmap.Low
	}
	return sev
...
func NewHTTPRepository(
	...
	return &VulnmapCodeHTTPClient{client, instrumentor, errorReporter}
}
...
func (s *VulnmapCodeHTTPClient) doCall(ctx context.Context,
	method string,
	path string,
	requestBody []byte,
	...
	for i := 0; i < retryCount; i++ {
		...
		log.Trace().Str("requestBody", string(requestBody)).Str("vulnmap-request-id", requestId).Msg("SEND TO REMOTE")

		response, body, err := s.httpCall(req)
		responseBody = body
		log.Trace().Str("response.Status", response.Status).
			Str("responseBody", string(responseBody)).
			Str("vulnmap-request-id", requestId).
...
func (s *VulnmapCodeHTTPClient) httpCall(req *http.Request) (*http.Response, []byte, error) {
	...
	defer func(Body io.ReadCloser) {
		err := Body.Close()
		if err != nil {
			log.Err(err).Msg("Couldn't close response body in call to Vulnmap Code")
		}
	}(response.Body)
...
func (s *VulnmapCodeHTTPClient) newRequest(
	c *config.Config,
	method string,
	path string,
	body *bytes.Buffer,
...
func (s *VulnmapCodeHTTPClient) addDefaultHeaders(req *http.Request, requestId string, method string) {
	req.Header.Set("vulnmap-request-id", requestId)
...
func (s *VulnmapCodeHTTPClient) addOrganization(c *config.Config, req *http.Request) {
	// Setting a chosen org name for the request
	org := c.Organization()
	if org != "" {
		req.Header.Set("vulnmap-org-name", org)
	}
}

func (s *VulnmapCodeHTTPClient) encodeIfNeeded(method string, requestBody []byte) (*bytes.Buffer, error) {
	b := new(bytes.Buffer)
...
func (s *VulnmapCodeHTTPClient) mustBeEncoded(method string) bool {
...
func (s *VulnmapCodeHTTPClient) RunAnalysis(
	...
	if response.Status == failed.message {
		...
		return nil, failed, VulnmapAnalysisFailedError{Msg: string(responseBody)}
	}
...
func (s *VulnmapCodeHTTPClient) analysisRequestBody(options *AnalysisOptions) ([]byte, error) {
...
func (s *VulnmapCodeHTTPClient) checkResponseCode(r *http.Response) error {
...
func (s *VulnmapCodeHTTPClient) RunAutofix(
	...
	if response.Status == failed.message {
		...
		return nil, failed, VulnmapAutofixFailedError{Msg: string(responseBody)}
	}
...
func (s *VulnmapCodeHTTPClient) autofixRequestBody(options *AutofixOptions) ([]byte, error) {
	_, ruleID, ok := getIssueLangAndRuleId(options.issue)
	if !ok {
		return nil, VulnmapAutofixFailedError{Msg: "Issue's ruleID does not follow <lang>/<ruleKey> format"}
	}
...
func getCodeApiUrl(c *config.Config) (string, error) {
	if !c.IsFedramp() {
		return c.VulnmapCodeApi(), nil
	}
	url, err := url.Parse(c.VulnmapCodeApi())
	if err != nil {
		return "", err
	}
