...
func (sc *Scanner) IsEnabled() bool {
	...
	return currentConfig().IsVulnmapCodeEnabled() ||
		currentConfig().IsVulnmapCodeQualityEnabled() ||
		currentConfig().IsVulnmapCodeSecurityEnabled()
}
...
func (sc *Scanner) SupportedCommands() []vulnmap.CommandName {
	return []vulnmap.CommandName{vulnmap.NavigateToRangeCommand}
}

func (sc *Scanner) Scan(ctx context.Context, path string, folderPath string) (issues []vulnmap.Issue, err error) {
	sastResponse, err := sc.VulnmapApiClient.SastSettings()
	method := "Scan"
	...
	if isAlreadyWaiting {
		return []vulnmap.Issue{}, nil // Returning an empty slice implies that no vulnerabilities were found
	}
	defer func() {
	...
	t.BeginWithMessage("Vulnmap Code: Collecting files in \""+folderPath+"\"", "Evaluating ignores and counting files...")
...
func (sc *Scanner) UploadAndAnalyze(ctx context.Context,
	...
	if uploadedBundle.BundleHash == "" {
		log.Info().Msg("empty bundle, no Vulnmap Code analysis")
		return issues, nil
	}
...
func (sc *Scanner) createBundle(ctx context.Context,
	...
	b = Bundle{
		VulnmapCode:      sc.BundleUploader.VulnmapCode,
		Files:         bundleFiles,
	...
	if len(fileHashes) > 0 {
		b.BundleHash, b.missingFiles, err = sc.BundleUploader.VulnmapCode.CreateBundle(span.Context(), fileHashes)
	}
	return b, err
}
