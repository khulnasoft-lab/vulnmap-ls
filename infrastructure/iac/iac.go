...
import (
	...
	"github.com/khulnasoft-lab/vulnmap-ls/internal/scans"
	"github.com/khulnasoft-lab/vulnmap-ls/internal/uri"
)

var scanCount = 1
var _ vulnmap.ProductScanner = (*Scanner)(nil)
...
func (iac *Scanner) IsEnabled() bool {
	return config.CurrentConfig().IsVulnmapIacEnabled()
}
...
func (iac *Scanner) SupportedCommands() []vulnmap.CommandName {
	return []vulnmap.CommandName{}
}

func (iac *Scanner) Scan(ctx context.Context, path string, _ string) (issues []vulnmap.Issue, err error) {
	if ctx.Err() != nil {
	...
	p.BeginUnquantifiableLength("Scanning for Vulnmap IaC issues", path)
	defer p.EndWithMessage("Vulnmap Iac Scan completed.")
	...
	if err != nil {
		...
		case *exec.ExitError:
			...
			ERR:
				...
				log.Err(err).Str("method", method).Str("output", errorOutput).Msg("Error while calling Vulnmap CLI")
				err = errors.Wrap(err, fmt.Sprintf("Error executing %v.\n%s", cmd, errorOutput))
				return nil, err
			}
		default:
			log.Err(err).Str("method", method).Msg("Error while calling Vulnmap CLI")
			return nil, err
		}
	}
...
func (iac *Scanner) retrieveAnalysis(scanResult iacScanResult, workspacePath string) ([]vulnmap.Issue, error) {
	...
	for _, issue := range scanResult.IacIssues {
		...
		if err != nil {
			return nil, errors.Wrap(err, "unable to convert IaC issue to Vulnmap issue")
		}
...
func (iac *Scanner) toIssue(affectedFilePath string, issue iacIssue, fileContent string) (vulnmap.Issue, error) {
	...
	codeActionTitle := fmt.Sprintf("Open description of '%s' in browser (Vulnmap)", issue.Title)
	...
	action, err := vulnmap.NewCodeAction(codeActionTitle, nil, command)
	if err != nil {
	...
	if err != nil {
		return vulnmap.Issue{}, errors.Wrap(err, "unable to create IaC issue additional data")
	}

	return vulnmap.Issue{
		ID: issue.PublicID,
		Range: vulnmap.Range{
			Start: vulnmap.Position{Line: issue.LineNumber, Character: rangeStart},
			End:   vulnmap.Position{Line: issue.LineNumber, Character: rangeEnd},
		},
		Message:             fmt.Sprintf("%s (Vulnmap)", title),
		...
		CodeActions:         []vulnmap.CodeAction{action},
		AdditionalData:      additionalData,
	}, nil
}

func (iac *Scanner) toAdditionalData(affectedFilePath string, issue iacIssue) (vulnmap.IaCIssueData, error) {
	key := getIssueKey(affectedFilePath, issue)

	iacIssuePath, err := parseIacIssuePath(issue.Path)
	if err != nil {
		return vulnmap.IaCIssueData{}, errors.Wrap(err, "unable to parse IaC issue path")
	}

	return vulnmap.IaCIssueData{
		Key:           key,
		Title:         issue.Title,
...
func newIacCommand(codeActionTitle string, issueURL *url.URL) *vulnmap.CommandData {
...
func (iac *Scanner) toIssueSeverity(vulnmapSeverity string) vulnmap.Severity {
	severity, ok := issueSeverities[vulnmapSeverity]
	if !ok {
