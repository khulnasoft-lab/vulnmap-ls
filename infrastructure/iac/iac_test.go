...
func Test_retrieveIssues_IgnoresParsingErrors(t *testing.T) {
	...
	issues, err := scanner.retrieveIssues(results, []vulnmap.Issue{}, "")
...
func Test_createIssueDataForCustomUI_SuccessfullyParses(t *testing.T) {
	...
	expectedAdditionalData := vulnmap.IaCIssueData{
		...
		Documentation: "https://security.vulnmap.khulnasoft.com/rules/cloud/PublicID",
...
func Test_parseIacResult(t *testing.T) {
	...
	retrieveIssues, err := scanner.retrieveIssues(issues, []vulnmap.Issue{}, ".")
	assert.NoError(t, err)
