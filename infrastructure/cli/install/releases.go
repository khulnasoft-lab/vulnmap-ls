...
const defaultBaseURL = "https://static.vulnmap.khulnasoft.com"
...
type ReleaseAssets struct {
	AlpineLinux *ReleaseAsset `json:"vulnmap-alpine,omitempty"`
	Linux       *ReleaseAsset `json:"vulnmap-linux,omitempty"`
	LinuxARM64  *ReleaseAsset `json:"vulnmap-linux-arm64,omitempty"`
	MacOS       *ReleaseAsset `json:"vulnmap-macos,omitempty"`
	MacOSARM64  *ReleaseAsset `json:"vulnmap-macos-arm64,omitempty"`
...
func (r *CLIRelease) GetLatestRelease(ctx context.Context) (*Release, error) {
	...
	log.Ctx(ctx).Trace().Str("url", releaseURL).Msg("requesting version for Vulnmap CLI")
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("failed to obtained Vulnmap CLI release from %q: %s ", releaseURL, resp.Status)
	}
