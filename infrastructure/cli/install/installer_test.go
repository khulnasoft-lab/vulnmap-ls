...
func Test_Find_CliPathInSettings_CliPathFound(t *testing.T) {
	// Arrange
	testutil.IntegTest(t)
	file, err := os.CreateTemp(t.TempDir(), "vulnmap-win.exe")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	cliPath := file.Name()
	t.Setenv("PATH", "")
	t.Setenv("VULNMAP_TOKEN", "")
	t.Setenv("VULNMAP_CLI_PATH", "")
	...
	r := &Release{
		Assets: &ReleaseAssets{
			AlpineLinux: &ReleaseAsset{
				ChecksumInfo: checksumString + "  vulnmap-alpine",
			},
			Linux: &ReleaseAsset{
				ChecksumInfo: checksumString + "  vulnmap-linux",
			},
			LinuxARM64: &ReleaseAsset{
				ChecksumInfo: checksumString + "  vulnmap-linux-arm64",
			},
			MacOS: &ReleaseAsset{
				ChecksumInfo: checksumString + "  vulnmap-macos",
			},
			Windows: &ReleaseAsset{
				ChecksumInfo: checksumString + "  vulnmap-win.exe",
			},
		},
	}
