...
func NewInitializer(
	authenticator vulnmap.AuthenticationService,
	errorReporter error_reporting.ErrorReporter,
	analytics ux.Analytics,
	notifier noti.Notifier,
) *Initializer {
...
func (i *Initializer) Init() error {
	...
	if currentConfig.NonEmptyToken() {
		cmd, _ := command.CreateFromCommandData(vulnmap.CommandData{CommandId: vulnmap.GetActiveUserCommand}, nil, i.authenticationService, nil, i.notifier, nil, nil)
...
func (i *Initializer) authenticate(authenticationService vulnmap.AuthenticationService, errorMessage string) error {
	i.notifier.SendShowMessage(sglsp.Info, "Authenticating to Vulnmap. This could open a browser window.")

	token, err := authenticationService.Authenticate(context.Background())
	if token == "" || err != nil {
		if err == nil {
			err = &vulnmap.AuthenticationFailedError{}
		}
		i.notifier.SendError(err)
...
func (i *Initializer) handleNotAuthenticatedAndManualAuthActive() error {
	err := &vulnmap.AuthenticationFailedError{ManualAuthentication: true}
	i.notifier.SendError(err)
