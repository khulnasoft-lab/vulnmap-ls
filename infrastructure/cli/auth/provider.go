...
func (a *CliAuthenticationProvider) ClearAuthentication(ctx context.Context) error {
	...
	str := out.String()
	log.Info().Str("output", str).Msg("unset Vulnmap CLI API token")
...
// Auth represents the `vulnmap auth` command.
func (a *CliAuthenticationProvider) authenticate(ctx context.Context) error {
	...
	go func() {
		...
		log.Info().Str("method", "authenticate").Str("output", out.String()).Msg("auth Vulnmap CLI")
	}()
...
func (a *CliAuthenticationProvider) getToken(ctx context.Context) (string, error) {
	...
	if token == "" {
		return "", vulnmap.ErrEmptyAPIToken
	}
...
func (a *CliAuthenticationProvider) buildCLICmd(ctx context.Context, args ...string) *exec.Cmd {
	...
	log.Info().Str("command", cmd.String()).Interface("env", cmd.Env).Msg("running Vulnmap CLI command")
	return cmd
}
...
func (a *CliAuthenticationProvider) runCLICmd(ctx context.Context, cmd *exec.Cmd) error {
	...
	if err != nil {
		log.Err(err).Msg("error while calling Vulnmap CLI command")
	}
