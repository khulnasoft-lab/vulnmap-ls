...
func TestSetAuthURLCmd(t *testing.T) {
	t.Run("works for the default endpoint", func(t *testing.T) {
		...
		var expectedURL = "https://app.vulnmap.khulnasoft.com/login?token=<TOKEN>&utm_medium=cli&utm_source=cli&utm_campaign=cli&os=darwin&docker=false"
	...
	t.Run("works when URL is in a substring", func(t *testing.T) {
		...
		var stringWithURL = "If auth does not automatically redirect you, copy this auth link: https://app.vulnmap.khulnasoft.com/login?token=<TOKEN>&utm_medium=cli&utm_source=cli&utm_campaign=cli&os=darwin&docker=false"
		var expectedURL = "https://app.vulnmap.khulnasoft.com/login?token=<TOKEN>&utm_medium=cli&utm_source=cli&utm_campaign=cli&os=darwin&docker=false"
...
func TestBuildCLICmd(t *testing.T) {
	...
	t.Run("Api endpoint is respected", func(t *testing.T) {
		...
		ctx := context.Background()
		provider := &CliAuthenticationProvider{}
		config.CurrentConfig().UpdateApiEndpoints("https://app.vulnmap.khulnasoft.com/api")

		cmd := provider.buildCLICmd(ctx, "auth")

		assert.Contains(t, cmd.Env, "VULNMAP_API=https://app.vulnmap.khulnasoft.com/api")
	})
	...
	t.Run("Telemetry disabled setting is respected", func(t *testing.T) {
		...
		assert.Contains(t, cmd.Env, "VULNMAP_CFG_DISABLE_ANALYTICS=1")
	})
	...
	t.Run("Telemetry env var isn't set if telemetry enabled", func(t *testing.T) {
		...
		assert.NotContains(t, cmd.Env, "VULNMAP_CFG_DISABLE_ANALYTICS=1")
	})
}
