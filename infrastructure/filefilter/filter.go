...
func NewFileFilter(rootFolder string, logger *zerolog.Logger) *FileFilter {
	return &FileFilter{
		repoRoot:       rootFolder,
		ignoreFiles:    []string{".gitignore", ".dcignore", ".vulnmap"},
...
func (f *FileFilter) collectGlobs(path string) []string {
	...
	for _, ignoreFile := range f.ignoreFiles {
		...
		if fileFound {
			...
			if filepath.Base(ignoreFilePath) == ".vulnmap" { // .vulnmap files are yaml files and should be parsed differently
				parsedRules, err := parseDotVulnmapFile(content, folderPath)
				globs = append(globs, parsedRules...)
				if err != nil {
					f.logger.Err(err).Msg("Can't parse .vulnmap file")
				}
...
func parseDotVulnmapFile(content []byte, baseDir string) ([]string, error) {
	type DotVulnmapRules struct {
		Exclude struct {
...
func parseIgnoreRuleToGlobs(rule string, baseDir string) (globs []string) {
	// Shamelessly stolen from code-client: https://github.com/khulnasoft-lab/code-client/blob/7a9e5cdbed4e8a6a0f2597fcd64b67800279e585/src/files.ts#L67
