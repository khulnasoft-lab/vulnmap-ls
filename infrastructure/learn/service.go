...
func (s *serviceImpl) GetLesson(ecosystem string, rule string, cwes []string, cves []string, issueType vulnmap.Type) (lesson *Lesson, err error) {
...
func (s *serviceImpl) lessonsLookupParams(
	...
	switch issueType {
	case vulnmap.DependencyVulnerability:
		params = &LessonLookupParams{
			rule,
			ecosystem,
			cwes,
	...
	case vulnmap.CodeSecurityVulnerability:
...
func (s *serviceImpl) LearnEndpoint(conf *config.Config) (learnEndpoint string, err error) {
	...
	if err != nil {
		logger.Err(err).Msg("failed to parse Vulnmap API URL")
