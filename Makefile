...
LDFLAGS_DEV := "-X 'github.com/khulnasoft-lab/vulnmap-ls/application/config.Development=true' -X 'github.com/khulnasoft-lab/vulnmap-ls/application/config.Version=v$(VERSION)-SNAPSHOT-$(COMMIT)'"
...
proxy-test:
	...
	@docker build -t "vulnmap-ls:$(VERSION)" -f .github/docker-based-tests/Dockerfile .
	@docker run --rm --cap-add=NET_ADMIN --name "vulnmap-ls" --env "VULNMAP_TOKEN=$(VULNMAP_TOKEN)" vulnmap-ls:$(VERSION) go test -failfast $(NOCACHE) $(TIMEOUT) ./...
...
.PHONY: run
run:
	@echo "==> Running Vulnmap LS server..."
...
license-update:
	...
	@rm -rf licenses
	@go-licenses save . --save_path="licenses" --ignore "github.com/khulnasoft-lab/vulnmap-ls"
