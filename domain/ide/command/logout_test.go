...
func TestLogoutCommand_Execute_ClearsIssues(t *testing.T) {
	...
	authenticationService := vulnmap.NewAuthenticationService(
		provider,
		ux.NewTestAnalytics(),
	...
	cmd := logoutCommand{
		command:     vulnmap.CommandData{CommandId: vulnmap.LogoutCommand},
		authService: authenticationService,
	}

	scanner := vulnmap.NewTestScanner()
	scanner.Issues = []vulnmap.Issue{{ID: "issue-1"}}
