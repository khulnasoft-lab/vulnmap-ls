...
func Test_Scan_WhenNoIssues_shouldNotProcessResults(t *testing.T) {
	...
	testutil.UnitTest(t)
	f := NewFolder("dummy", "dummy", vulnmap.NewTestScanner(), hoverRecorder, vulnmap.NewMockScanNotifier(), notification.NewNotifier())
...
func TestProcessResults_SendsDiagnosticsAndHovers(t *testing.T) {
	...
	f := NewFolder("dummy", "dummy", vulnmap.NewTestScanner(), hoverService, vulnmap.NewMockScanNotifier(), notification.NewNotifier())
...
func Test_ProcessResults_whenSamePaths_AccumulatesIssues(t *testing.T) {
	...
	data.Issues = []vulnmap.Issue{NewMockIssue("id3", "path1")}
	f.processResults(data)
...
func TestProcessResults_whenFilteringSeverity_ProcessesOnlyFilteredIssues(t *testing.T) {
	...
	data := vulnmap.ScanData{
		...
		Issues: []vulnmap.Issue{
			...
			NewMockIssueWithSeverity("id3", "path1", vulnmap.Medium),
			NewMockIssueWithSeverity("id4", "path1", vulnmap.Low),
			NewMockIssueWithSeverity("id5", "path1", vulnmap.Critical),
		},
	}
	f.processResults(data)
...
func Test_IsTrusted_shouldReturnTrueForSubfolderOfTrustedFolders_Linux(t *testing.T) {
	...
	config.CurrentConfig().SetTrustedFolderFeatureEnabled(true)
	config.CurrentConfig().SetTrustedFolders([]string{"/dummy"})
	f := NewFolder("/dummy/dummyF", "dummy", vulnmap.NewTestScanner(), hover.NewFakeHoverService(), vulnmap.NewMockScanNotifier(), notification.NewNotifier())
	assert.True(t, f.IsTrusted())
}
...
func Test_IsTrusted_shouldReturnFalseForDifferentFolder(t *testing.T) {
	...
	config.CurrentConfig().SetTrustedFolderFeatureEnabled(true)
	config.CurrentConfig().SetTrustedFolders([]string{"/dummy"})
	f := NewFolder("/UntrustedPath", "dummy", vulnmap.NewTestScanner(), hover.NewFakeHoverService(), vulnmap.NewMockScanNotifier(), notification.NewNotifier())
	assert.False(t, f.IsTrusted())
}
...
func Test_IsTrusted_shouldReturnTrueForSubfolderOfTrustedFolders(t *testing.T) {
	...
	f := NewFolder("c:\\dummy\\dummyF", "dummy", vulnmap.NewTestScanner(), hover.NewFakeHoverService(), vulnmap.NewMockScanNotifier(), notification.NewNotifier())
	assert.True(t, f.IsTrusted())
}
...
func Test_FilterCachedDiagnostics_filtersDisabledSeverity(t *testing.T) {
	...
	criticalIssue := vulnmap.Issue{AffectedFilePath: filePath, Severity: vulnmap.Critical, Product: product.ProductOpenSource}
	highIssue := vulnmap.Issue{AffectedFilePath: filePath, Severity: vulnmap.High, Product: product.ProductOpenSource}
	mediumIssue := vulnmap.Issue{AffectedFilePath: filePath, Severity: vulnmap.Medium, Product: product.ProductOpenSource}
	lowIssue := vulnmap.Issue{AffectedFilePath: filePath, Severity: vulnmap.Low, Product: product.ProductOpenSource}
	scannerRecorder := vulnmap.NewTestScanner()
	scannerRecorder.Issues = []vulnmap.Issue{
		criticalIssue,
		highIssue,
		mediumIssue,
		lowIssue,
...
func Test_processResults_ShouldCountSeverityByProduct(t *testing.T) {
	...
	scanData := vulnmap.ScanData{
		...
		Issues: []vulnmap.Issue{
			{Severity: vulnmap.Critical, Product: product.ProductOpenSource},
			{Severity: vulnmap.Critical, Product: product.ProductOpenSource},
			{Severity: vulnmap.High, Product: product.ProductOpenSource},
			{Severity: vulnmap.High, Product: product.ProductOpenSource},
			{Severity: vulnmap.Critical, Product: product.ProductInfrastructureAsCode}, // SeverityCount incremented by ScanData.Product
		},
	}
...
func NewMockFolderWithScanNotifier(notifier noti.Notifier) (*Folder, *vulnmap.MockScanNotifier) {
	scanNotifier := vulnmap.NewMockScanNotifier()
	return NewFolder("dummy", "dummy", vulnmap.NewTestScanner(), hover.NewFakeHoverService(), scanNotifier, notifier), scanNotifier
}

func NewMockIssue(id, path string) vulnmap.Issue {
...
func NewMockIssueWithSeverity(id, path string, severity vulnmap.Severity) vulnmap.Issue {
