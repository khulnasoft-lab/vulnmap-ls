...
var (
	...
	supportedCommands = []string{
		vulnmap.WorkspaceScanCommand,
		vulnmap.OpenBrowserCommand,
		vulnmap.NavigateToRangeCommand,
		vulnmap.LoginCommand,
	}
)
...
func Test_initialize_shouldDefaultToTokenAuthentication(t *testing.T) {
	...
	assert.Equal(t, "*vulnmap.FakeAuthenticationProvider", reflect.TypeOf(di.AuthenticationService().Provider()).String())
}
...
func Test_initialize_shouldSupportAllCommands(t *testing.T) {
	...
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.NavigateToRangeCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.WorkspaceScanCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.WorkspaceFolderScanCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.OpenBrowserCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.LoginCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.CopyAuthLinkCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.LogoutCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.TrustWorkspaceFoldersCommand)
	assert.Contains(t, result.Capabilities.ExecuteCommandProvider.Commands, vulnmap.GetLearnLesson)
...
func Test_CodeActionResolve_ShouldExecuteCommands(t *testing.T) {
	...
	expected := vulnmap.OpenBrowserCommand
	serviceMock := vulnmap.NewCommandServiceMock()
	...
	_, err = loc.Client.Call(ctx, "codeAction/resolve", lsp.CodeAction{
		...
		Command: &sglsp.Command{
			Title:     expected,
			Command:   expected,
			Arguments: []any{"https://vulnmap.khulnasoft.com"},
		},
	})
	if err != nil {
		t.Fatal(err)
...
func Test_SmokeWorkspaceScanIacAndCode(t *testing.T) {
	iacFile := "main.tf"
	codeFile := "app.js"
	runSmokeTest("https://github.com/deepcodeg/vulnmapcon-goof.git", "eba8407", iacFile, codeFile, t)
}
...
func runSmokeTest(repo string, commit string, file1 string, file2 string, t *testing.T) {
	...
	config.CurrentConfig().SetVulnmapCodeEnabled(true)
	config.CurrentConfig().SetVulnmapIacEnabled(true)
	config.CurrentConfig().SetVulnmapOssEnabled(true)
	jsonRPCRecorder.ClearCallbacks()
	jsonRPCRecorder.ClearNotifications()
	cleanupChannels()
	di.Init()
	...
	clientParams := lsp.InitializeParams{
		...
		InitializationOptions: lsp.Settings{
			Endpoint:                    os.Getenv("VULNMAP_API"),
...
func Test_SmokeVulnmapCodeFileScan(t *testing.T) {
	loc := setupServer(t)
	testutil.SmokeTest(t)
	config.CurrentConfig().SetVulnmapCodeEnabled(true)
	jsonRPCRecorder.ClearCallbacks()
