...
func Test_NotifierShouldSendNotificationToClient(t *testing.T) {
	...
	assert.Eventually(
		t,
		func() bool {
			notifications := jsonRPCRecorder.FindNotificationsByMethod("$/vulnmap.hasAuthenticated")
...
func Test_IsAvailableCliNotification(t *testing.T) {
	...
	var expected = lsp.VulnmapIsAvailableCli{CliPath: "path"}

	di.Notifier().Send(expected)
	assert.Eventually(
		t,
		func() bool {
			notifications := jsonRPCRecorder.FindNotificationsByMethod("$/vulnmap.isAvailableCli")
			if len(notifications) < 1 {
				return false
			}
			for _, n := range notifications {
				var actual = lsp.VulnmapIsAvailableCli{}
				_ = n.UnmarshalParams(&actual)
				if reflect.DeepEqual(expected, actual) {
					return true
				}
...
func TestShowMessageRequest(t *testing.T) {
	t.Run("should send request to client", func(t *testing.T) {
		...
		// data, err := command.CreateFromCommandData(vulnmap.CommandData{
		...
		actionCommandMap.Add(
			vulnmap.MessageAction(expectedTitle),
			data,
		)

		expected := vulnmap.ShowMessageRequest{Message: "message", Type: vulnmap.Info, Actions: actionCommandMap}
		...
		assert.Eventually(
			...
			func() bool {
				...
				_, ok := expected.Actions.Get(vulnmap.MessageAction(expectedTitle))
				return ok &&
	...
	t.Run("should execute a command when action item is selected", func(t *testing.T) {
		...
		actionCommandMap.Add(vulnmap.MessageAction(selectedAction), vulnmap.CommandData{CommandId: vulnmap.OpenBrowserCommand, Arguments: []any{"https://vulnmap.khulnasoft.com"}})

		request := vulnmap.ShowMessageRequest{Message: "message", Type: vulnmap.Info, Actions: actionCommandMap}
		...
		assert.Eventually(
			...
			func() bool {
				...
				commandServiceMock := commandService.(*vulnmap.CommandServiceMock)
				return commandServiceMock.ExecutedCommands()[0].CommandId == vulnmap.OpenBrowserCommand
			},
			2*time.Second,
			10*time.Millisecond,
		)
