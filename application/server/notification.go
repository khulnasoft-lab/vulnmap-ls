...
func registerNotifier(srv lsp.Server) {
	callbackFunction := func(params any) {
		...
		case lsp.AuthenticationParams:
			notifier(srv, "$/vulnmap.hasAuthenticated", params)
			log.Info().Str("method", "registerNotifier").
				Msg("sending token")
		case lsp.VulnmapIsAvailableCli:
			notifier(srv, "$/vulnmap.isAvailableCli", params)
			log.Info().Str("method", "registerNotifier").
		...
		case lsp.VulnmapTrustedFoldersParams:
			notifier(srv, "$/vulnmap.addTrustedFolders", params)
			log.Info().
				Str("method", "registerNotifier").
		...
		case vulnmap.ShowMessageRequest:
...
func handleShowMessageRequest(srv lsp.Server, params vulnmap.ShowMessageRequest) {
	...
	if callback != nil {
		...
		selectedCommand, ok := params.Actions.Get(vulnmap.MessageAction(actionItem.Title))
		if !ok {
