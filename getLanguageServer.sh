...
VERSION=$(curl -sSL --compressed https://static.vulnmap.khulnasoft.com/vulnmap-ls/$PROTOCOL_VERSION/metadata.json | jq .version | sed -e s/\"//g)
DESTINATION="/usr/local/bin/vulnmap-ls"
DOWNLOAD_URL="https://static.vulnmap.khulnasoft.com/vulnmap-ls/$PROTOCOL_VERSION/vulnmap-ls_${VERSION}_${OS}_${ARCH}"

set +e
if [[ -f $DESTINATION ]]; then
  LS_VERSION=$($DESTINATION -v | xargs)
  echo "Vulnmap Language Server ($LS_VERSION) is already installed at $DESTINATION"
...
chmod +x "$DESTINATION"
echo
echo "✨🎉 Vulnmap Language Server $VERSION installed to $DESTINATION."
